...
if (!readConfig()) {
  process.stderr.write(
    '[namecheap-mcp] UNCONFIGURED at startup. sources: ' +
...
const UNCONFIGURED_INSTRUCTIONS =
  'namecheap-mcp is not yet authenticated. Call the `setup` tool to enter your ' +
...
const READY_INSTRUCTIONS =
  'namecheap-mcp is authenticated. Manage domains, DNS records, SSL certificates, ' +
...
function registerReadyTools(): void {
  if (readyToolsRegistered) return;
