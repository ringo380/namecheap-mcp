...
export class NamecheapClient {
     ...
     */
    async execute(command, params = {}, method = 'GET', retries = 0) {
        const authParams = {
        ...
        try {
            if (method === 'POST') {
                const body = new URLSearchParams(Object.fromEntries(Object.entries(allParams).map(([k, v]) => [k, String(v)])));
    ...
    splitDomain(domainName) {
        ...
        if (parts.length > 3) {
            throw new Error(`"${domainName}" appears to be a subdomain. Namecheap DNS tools require a registrable domain (e.g. "example.com" or "example.co.uk"), not a subdomain. Manage subdomains as host records within the zone.`);
        }
        return {
            sld: parts[0],
